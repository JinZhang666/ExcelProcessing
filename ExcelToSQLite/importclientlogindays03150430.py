...
def importClientLoginDays03150430():

    # 建立数据库的连接
    with sqlite3.connect('C:\sqlite\db\hxdata.db') as db:
        # ExcelDocument('..\input\营销人员和营业部列表.xlsx') as src:
        insert_template = "INSERT INTO clientlogindays03150430" \
                          "(PHONE, COUNT) " \
                          "VALUES (?, ?);"

        # 清空的数据库遗留的数据（选择）
        db.execute('DELETE FROM clientlogindays03150430;')

        workbookPath = '..\hisinput\\tradelogin\星途-奇点股票20190315-20190430登录天数统计.xlsx'
        df = pd.read_excel(workbookPath, sheet_name='星途-奇点股票20190315-20190430登录天数统计')
        ...
        # 检查是不是所有的数据都被加载了
        select_stmt = 'SELECT * FROM clientlogindays03150430;'
        row = 0
